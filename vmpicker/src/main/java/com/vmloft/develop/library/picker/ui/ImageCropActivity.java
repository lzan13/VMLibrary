...
import com.vmloft.develop.library.picker.util.BitmapUtil;
...
//        mCropImageView.setImageURI(Uri.fromFile(new File(imagePath)));
    ...
    @Override
    public void onBitmapSaveSuccess(ImageItem bean) {
        // 裁剪后替换掉返回数据的内容，但是不要改变全局中的选中数据
        mImageItems.remove(0);
        mImageItems.add(bean);
    ...
    @Override
    public void onBitmapSaveError(ImageItem bean) {
