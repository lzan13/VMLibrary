package com.vmloft.develop.library.picker.view;
...
//                    bitmap.recycle();
    ...
    public void saveBitmapToFile(File folder, int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        final ImageItem bean = new ImageItem();
     ...
     * 将图片保存在本地
     */
    private void saveOutput(Bitmap croppedImage, Bitmap.CompressFormat outputFormat, ImageItem bean) {
        OutputStream outputStream = null;
        try {
            outputStream = getContext().getContentResolver().openOutputStream(Uri.fromFile(new File(bean.path)));
    ...
    private static class InnerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ImageItem bean = (ImageItem) msg.obj;
            switch (msg.what) {
                case SAVE_SUCCESS:
                    if (mListener != null) mListener.onBitmapSaveSuccess(bean);
                    break;
                case SAVE_ERROR:
                    if (mListener != null) mListener.onBitmapSaveError(bean);
                    break;
            }
        }
    }
    ...
    public interface OnBitmapSaveCompleteListener {
        void onBitmapSaveSuccess(ImageItem bean);

        void onBitmapSaveError(ImageItem bean);
    }
