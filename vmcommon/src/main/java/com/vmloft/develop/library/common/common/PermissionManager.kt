...
class PermissionManager() {
    ...
    fun storagePermission(context: Context): Boolean {
        val bean = VMPermissionBean(
            ...
            "读写手机存储",
            "发送和保存图片需要读写手机存储，请允许我们访问读写手机存储权限，否则你将无法使用应用"
        )
    ...
    fun cameraPermission(context: Context): Boolean {
        val bean = VMPermissionBean(
            ...
            "访问相机",
            "拍摄照片需要访问相机，请允许我们获取访问相机权限，否则你将无法使用应用"
        )
        val result = checkPermission(context, bean)
        if (!result) {
            // 没有权限，去请求相机权限
    ...
    fun requestPermissions(context: Context?) {
        ...
        VMPermission.getInstance(context).setEnableDialog(false).setPermissionList(list)
